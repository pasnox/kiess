...
void PrivacyListItem::fromXml(const QDomElement& el)
{
	...
	if (el.isNull() || el.tagName() != "item") {
		qDebug() << "Invalid root tag for privacy list item.";
		return;
	}
	...
	if (type_ == JidType && XMPP::Jid(value_).isEmpty()) 
		qDebug() << "Invalid value for item of type 'jid'.";
	else if (type_ == GroupType && value_.isEmpty()) 
		qDebug() << "Empty value for item of type 'group'.";
	else if (type_ == SubscriptionType && value_ != "from" && value != "to" && value_ != "both" && value_ != "none")
		qDebug() << "Invalid value for item of type 'subscription'.";
	else if (type_ == FallthroughType && !value_.isEmpty()) 
		qDebug() << "Value given for item of fallthrough type.";
	...
	if (action == "allow") 
		...
		qDebug() << "Invalid action given for item.";

	bool ok;
	order_ = el.attribute("order").toUInt(&ok);
	if (!ok)
		qDebug() << "Invalid order value for item.";
