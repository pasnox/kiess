...
bool GetPrivacyListsTask::take ( const QDomElement &x ) {
	...
	//qDebug() << "Got reply for privacy lists.";
	if ( x.attribute ( "type" ) == "result" ) {
		...
		for ( QDomNode n = q.firstChild(); !n.isNull(); n = n.nextSibling() ) {
			...
			if ( e.tagName() == "active" )
				...
				qDebug() << "Unknown tag in privacy lists.";
...
void SetPrivacyListsTask::onGo() {
	...
	QDomElement e;
	if ( changeDefault_ ) {
		//qDebug() << "Changing default privacy list.";
		e = doc()->createElement ( "default" );
		if ( !value_.isEmpty() )
			e.setAttribute ( "name",value_ );
	}
	else if ( changeActive_ ) {
		//qDebug() << "Changing active privacy list.";
		e = doc()->createElement ( "active" );
		if ( !value_.isEmpty() )
			e.setAttribute ( "name",value_ );
	}
	else if ( changeList_ ) {
		//qDebug() << "Changing privacy list.";
		e = list_.toXml ( *doc() );
	}
	else {
		qDebug() << "Empty active/default list change request.";
		return;
	}
...
bool SetPrivacyListsTask::take ( const QDomElement &x ) {
	...
	if ( x.attribute ( "type" ) == "result" ) {
		//qDebug() << "Got successful reply for list change.";
		setSuccess();
	}
	else {
		qDebug() << "Got error reply for list change.";
		setError ( x );
	}
	return true;
}
...
bool GetPrivacyListTask::take ( const QDomElement &x ) {
	...
	//qDebug() << qPrintable (QString("Got privacy list %1 reply.").arg(name_));
	if ( x.attribute ( "type" ) == "result" ) {
...
void PrivacyManager::receiveLists()
{
	...
	if ( t->success() ) {
		...
		qDebug() << "Error in lists receiving.";
		emit listsError();
	}
}
...
void PrivacyManager::receiveList()
{
	...
	if ( t->success() ) {
		...
		qDebug() << "Error in list receiving.";
		emit listError();
	}
}
